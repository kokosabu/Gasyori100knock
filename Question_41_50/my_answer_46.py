...
table = np.pad(table, (1, 1), 'edge')
for y in range(rmax):
    for x in range(180):
        # Hough変換-2-1 表において、周囲8マス(8近傍)より注目ピクセルの得票数が多ければそのまま。
        out_table[y, x] = table[y+pad, x+pad]
        for r in range(8):
            # Hough変換-2-2 注目ピクセルの値が少なければ0にする。
            if table[y+pad, x+pad] < table[y+pad+round8[r][0], x+pad+round8[r][1]]:
                out_table[y, x] = 0

out = np.zeros((rmax, 180), dtype=np.int)
a = -np.sort(-out_table.reshape(rmax*180))[:10]
for i in range(10):
    out[out_table == a[i]] = 255
#cv2.imwrite("my_answer_45.jpg", out.astype(np.uint8))

# Hough変換-3 極大点のr, tの値をHough逆変換して検出した直線のパラメータを得る。
for i in range(10):
    r = np.where(out_table == a[i])[0][0]
    t = np.where(out_table == a[i])[1][0]
    for y in range(H):
        x = -sintable[t] / costable[t] * y + r / costable[t]
        if x > (W-1) or x < 0:
            continue
        img[y, int(x)] = np.array((0, 0, 255))
    for x in range(W):
        y = -costable[t] / sintable[t] * x + r / sintable[t]
        if y > (H-1) or y < 0:
            continue
        img[int(y), x] = np.array((0, 0, 255))

cv2.imwrite("my_answer_46.jpg", img.astype(np.uint8))
